...
class SearchTocUseCaseTest : SearchIntegrationTestBase() {
    ...
    @Test
    fun `buildTocTreeForBook returns tree with root entries`() = runBlocking {
        skipIfNoIndex()
    ...
    @Test
    fun `buildTocTreeForBook returns empty tree for book without TOC`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
    ...
    @Test
    fun `ensureTocLineIndex caches results`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
    ...
    @Test
    fun `ensureTocLineIndex creates valid index with line mappings`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
    ...
    @Test
    fun `collectLineIdsForTocSubtree returns lines for TOC entry`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
    ...
    @Test
    fun `getSubtreeLineIds returns LongArray`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
    ...
    @Test
    fun `computeTocCountsForLines returns counts with ancestor propagation`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
    ...
    @Test
    fun `clearCache removes cached indexes`() = runBlocking {
        skipIfNoIndex()
        val useCase = SearchTocUseCase(requireRepository())
