...
class SearchStatePersistenceUseCaseTest {
    ...
    private fun createTestResult(bookId: Long, lineId: Long): SearchResult =
        SearchResult(
            bookId = bookId,
    ...
    private fun createTestCategory(id: Long, title: String, parentId: Long? = null): Category =
        Category(
            id = id,
            title = title,
            level = 0,
    ...
    private fun createTestBook(id: Long, title: String, categoryId: Long): Book =
        Book(
            id = id,
            title = title,
    ...
    private fun createTestTocEntry(id: Long, text: String, bookId: Long, parentId: Long? = null): TocEntry =
        TocEntry(
            id = id,
            text = text,
    ...
    fun `buildSnapshot creates snapshot with results`() {
        ...
        val snapshot = useCase.buildSnapshot(
            results = results,
    ...
    fun `buildSnapshot handles nested search tree categories`() {
        ...
        assertEquals(2, snapshot.searchTree!![0].children[0].category.id)
    }
}
